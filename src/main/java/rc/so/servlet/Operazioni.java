...
import static rc.so.action.ActionB.checkfileuploadmultiplo;
import static rc.so.action.ActionB.delAllAllegatiDocenti;
import static rc.so.action.ActionB.delAllegatiDocenti;
import static rc.so.action.ActionB.delAllegatoB1Field;
import static rc.so.action.ActionB.domandaAnnullata;
import static rc.so.action.ActionB.esisteAllegatoB1;
import static rc.so.action.ActionB.generateUsername;
import static rc.so.action.ActionB.getSino;
import static rc.so.action.ActionB.insAllegatoA;
import static rc.so.action.ActionB.insAllegatoB;
import static rc.so.action.ActionB.insAllegatoB1;
import static rc.so.action.ActionB.insertAllegatoC2;
import static rc.so.action.ActionB.insertDocUserBando;
import static rc.so.action.ActionB.isPresenteUsername;
import static rc.so.action.ActionB.preparefileforupload;
import static rc.so.action.ActionB.remDocConvenzioni;
import static rc.so.action.ActionB.removeSingleDocUserBando;
import static rc.so.action.ActionB.setStatoDomandaAccRif;
import static rc.so.action.ActionB.setStatoInvioDocumenti;
import static rc.so.action.ActionB.settaInvioEmailROMA;
import static rc.so.action.ActionB.trackingAction;
import static rc.so.action.ActionB.verificaBando;
import static rc.so.action.ActionB.verificaInvioConvenzione;
import rc.so.action.Constant;
import static rc.so.action.Constant.bando;
import static rc.so.action.Constant.checkRegistrazione;
import static rc.so.action.Constant.datainizio;
import static rc.so.action.Constant.timestampSQL;
import static rc.so.action.Pdf_new.allegatoB1;
...
import static rc.so.otp.OTP.verificaOTP;
import static rc.so.otp.Sms.sendSMS2022;
import static rc.so.util.GoogleRecaptcha.isValid;
import static rc.so.util.SendMailJet.sendMail;
import static rc.so.util.SendMailJet.sendMailListAttach;
import static rc.so.util.Utility.createDir;
import static rc.so.util.Utility.estraiEccezione;
import static rc.so.util.Utility.getRequestCheckbox;
...
import static rc.so.util.Utility.utf8;
    ...
    protected void registraUtente(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (isMultipart) {
            try {
                ...
                System.out.println("rc.so.servlet.Operazioni.registraUtente() "+gRecaptchaResponse);
